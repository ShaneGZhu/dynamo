...
func TestDynamoGraphDeploymentValidator_Validate(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "v1beta1 native Rust EPP does not require configuration",
			deployment: betaDGDForAdmission(func(dgd *nvidiacomv1beta1.DynamoGraphDeployment) {
				worker := betaWorkerComponent(dgd)
				worker.ComponentType = nvidiacomv1beta1.ComponentTypeEPP
				worker.Replicas = k8sptr.To(int32(1))
				worker.RuntimeVersionOverride = "1.5.0"
				worker.PodTemplate.Spec.Containers[0].Image = "registry.example/dynamo-frontend:1.5.0"
			}),
		},
		{
			name: "v1beta1 native Rust EPP rejects legacy configuration",
			deployment: betaDGDForAdmission(func(dgd *nvidiacomv1beta1.DynamoGraphDeployment) {
				worker := betaWorkerComponent(dgd)
				worker.ComponentType = nvidiacomv1beta1.ComponentTypeEPP
				worker.Replicas = k8sptr.To(int32(1))
			...
			wantWebhookErrs: []string{"spec.components[1].eppConfig: Forbidden: must be omitted for native Rust EPP images with runtime version 1.5.0 or later"},
		},
