...
<Tabs>
  <Tab title="Codex">
    ...
    To drive a persistent Codex session over ACP, use the repository runner. Stock Dynamo is the safe default:
    ...
    The runner pins `@agentclientprotocol/codex-acp`, preserves the returned Codex thread ID across prompts, limits the child environment, and closes the ACP process when stdin closes.

    <Warning>
    Add `--session-final` only when the endpoint is explicitly running ThunderAgent. Stock frontends and KV routers do not have a ThunderAgent program lifecycle, and an implementation that does not recognize the terminal header could treat the envelope as ordinary model work.
    </Warning>

    Append `--session-final` for an explicitly selected ThunderAgent endpoint. After ACP closes, the runner sends one bounded terminal request for the created Codex thread and surfaces cleanup failure. See the [Codex well-lit path](https://github.com/ai-dynamo/dynamo/blob/main/examples/agent_harnesses/codex/README.md) for the complete command, dependency pins, verification, and cleanup behavior.
